...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.sagittal_slider = ctk.CTkSlider(self.image_frame, 
            from_=0, 
            to=100, 
            command=self.update_slice_sagittal
        )
        self.sagittal_slider_text = ctk.CTkLabel(self.image_frame, 
            text=f"Sagittal slice {self.sagittal_slice_idx}"
        )

        # Coronal slider widget
        self.coronal_slider = ctk.CTkSlider(self.image_frame, 
            from_=0, 
            to=671, 
            command=self.update_slice_coronal
        )
        self.coronal_slider_text = ctk.CTkLabel(self.image_frame, 
            text=f"Coronal slice {self.coronal_slice_idx}"
        )

        # Axial slider widget
        self.axial_slider = ctk.CTkSlider(self.image_frame, 
            from_=0, 
            to=671, 
            command=self.update_slice_axial
        )
        self.axial_slider_text = ctk.CTkLabel(self.image_frame, 
            text=f"Axial slice {self.axial_slice_idx}"
        )

        # Create tabview
        self.tabview = ctk.CTkTabview(self, width=300)
        self.tabview.grid(row=0, column=3, padx=(20, 10), pady=(20, 0), sticky="nsew")
        self.tabview.add("Tab 1")
        self.tabview.add("Tab 2")
        self.tabview.add("Tab 3")
        self.tabview.tab("Tab 1").grid_columnconfigure(0, weight=1) 
